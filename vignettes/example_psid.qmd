---
title: "Unit-level Event Studies with PSID"
subtitle: "Child Penalties on Participation"
...
sum_particip <- summary(mdl_particip,
    agg = "event_age",
...
compute_binsreg <- function(gender) {
    est <- unitdid_particip |>
        ...
        as.data.frame() |>
        binsreg(
            x = years_edu, y = particip_tilde, data = _,
            by = rel_time
        )

    map(0:5, ~ {
        est$data.plot[[paste0("Group ", .x)]]$data.dots |>
            select(x, fit) |>
            mutate(rel_time = .x)
    }) |>
        list_rbind() |>
